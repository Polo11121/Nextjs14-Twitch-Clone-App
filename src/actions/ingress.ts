...
export const createIngress = async (ingressType: IngressInput) => {
  ...
  await resetIngress(currentDbUser.id);

  const options: CreateIngressOptions = {
    name: currentDbUser.username,
    roomName: currentDbUser.id,
    participantName: currentDbUser.username,
    participantIdentity: currentDbUser.id,
  };
  ...
  revalidatePath(`/u/${currentDbUser.username}/keys`);
  return ingress;
};
